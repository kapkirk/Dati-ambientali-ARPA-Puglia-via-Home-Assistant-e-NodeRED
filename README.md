...
A [Home Assistant](https://home-assistant.io/) utility that helps you visualize the data of the main environmental indicators published by ARPA Puglia using [Node-RED](https://nodered.org/).

<br>

## Table of Contents

1. **[Project description](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#descrizione-del-progetto)**

2. **[Main features](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#caratteristiche-principali)**

3. **[Description of operation](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#descrizione-del-funzionamento)**

4. **[Installation](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#installazione)**
   1. **[Prerequisites](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#1---prerequisiti)**
   1. **[Configuring the data to be captured](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#2---configurazione-dei-dati-da-acquisire)**
   1. **[Home Assistant Setup](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#3---configurazione-home-assistant)**
   2. **[Configuring Node-RED](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#4---configurazione-di-node-red)**

5. **[Customizing NodeRed flows](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#personalizzazione-ed-analisi-dei-flussi-nodered)**

6. **[Code comment](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED#commentiamolo)**  
...
The main purpose of these lines is to automate the daily acquisition of environmental data published via API by the Regional Environmental Protection and Prevention Agency for Puglia, their processing through Node-RED and direct exposure in Home Assistant.
...
- **Node-RED Workflow**: Flow configured to query APIs daily, process data (formatting, validation) and send them directly to HA through sensors that self-generate through the NodeRED Companion app.
...
2. **PROCESSING AND SENDING IN HOME Assistant**:  
   The data is converted into sensors exposed directly in HA (structured es. JSON).
...
- Node-RED installed and configured with nodes:
  - `node-red-contrib-http-request`
- Home Assistant installed with the following HAKS dependencies:
  - Custom 'flex-table-card' downloadable (If you want to use the table view like I did)
  - Node-RED Companion installed (add-on component downloadable from HACS).
...
1. In the _Dashboard_ of the _lovelace_, where you prefer, open a new tab and paste the file code 'HA lovelace.txt'. Basically, just list them inside the card and the result will be as follows:
...
1. Copy and paste the contents of the 'Node-RED completo.json flow' file into a NodeRED flow:

'Node-RED completo.json flow':
...
The flows I have illustrated are obviously worthy of development. Personally, I have been responsible for collecting the data displayed by the control units near the areas where I live, but this does not mean that all the available values can be used, i.e. those shown in the "Main characteristics" table above. 
...
3. The 'csv' node receives the data and interprets it by dividing it into an _array_;
4. The 'function' node that follows, called 'Extracts measurement data', splits the received _array_ into separate strings producing more _payload_ for as many rows as there are transmitted by the control unit.
     ...
     9. After a few seconds you will get several answers:
        - the link to obtain the data, in the case we are dealing with, will be:

            https://dati.arpa.puglia.it/api/v1/measurements?language=ITA&format=CSV&id_station=104&debugMode=true

        -In the following section 'Response body' we will be able to read the following data:
```yaml
         <pre>=== measurements/executeQuery ===
formatParam: CSV
=== measurements/executeQuery ===
...
5. The 'function' node that follows, called 'divides messages for each pollutant', has as many outputs as there are lines in the _array_ received. So if a control unit exposes 6 pollutants, it will be necessary to modify it as follows:
   In the 'setup' tab, the number of outputs must be changed, which will be equal to the number of pollutants acquired;
   In the 'on message' tab you go:
   - modified the line 'if (measurements.length >= 9) {' and inserted the number of values to be acquired, those shown above, excluding the first line, so if there are rows of data we will insert 8;
   -inserted a strip 'let msg1 = { payload: measurements[0] };' for each pollutant exposed, starting from 'measurements[0]', like this:
```yaml
              let msg1 = { payload: misurazioni[0] }; // Primo oggetto
              let msg2 = { payload: misurazioni[1] }; // Secondo oggetto
              let msg3 = { payload: misurazioni[2] }; // Terzo oggetto
...
Obviously then we will also modify the subsequent 'return' message of the _payload_, inserting or deleting the 'msgx' that are needed or not, thus obtaining, for example:

```yaml 
          return [msg1, msg2, msg3, msg4, msg5, msg6, msg7, msg8, msg9];` 
```

in this way we will have an output for each pollutant to which we will connect, in the same order shown by the API, the sensor node that we will then find directly in HA. The sensor nodes, for greater clarity and speed, I have named them as provided by the short acronym used in the same regional API. Their configuration in Node-RED is as follows:
...
6. Before getting to the publication of the data, I had to insert a new 'function' node called 'format in value "value" in case of error' which has the purpose of replacing the data not received, exposed by the API with the wording 'null', changing them to '0'. This was necessary because otherwise Home Assistant is not able to interpret the data, thus returning a general error of the sensor that will not expose anything.
7. At the first launch of the created automation, we will be able to go to HA, and among the entities present in the 'settings/Devices/Node-RED Companion' tab we will be able to notice our newly created sensors. The structure will be identical for everyone, only the name of the sensor will change. We will then have the following visualization:

![HA1](https://github.com/kapkirk/Dati-ambientali-ARPA-Puglia-via-Home-Assistant-e-NodeRED/blob/main/images/Esposizione%20sensori%20in%20HA.png) 

with the following individual characteristics:
...
8. nothing else! Work done and have fun everyone!
